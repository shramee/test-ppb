 ...
 */
function pp_page_builder_Pootle_PB_Content_Block_admin_init() {
	global $pagenow;
	...
	if (
		...
		add_action( 'admin_head', 'Pootle_PB_Content_Block_load_tiny_mce' );
		add_filter( 'tiny_mce_before_init', 'Pootle_PB_Content_Block_init_editor', 20 );
		add_action( 'admin_print_scripts', 'Pootle_PB_Content_Block_scripts' );
		add_action( 'admin_print_styles', 'Pootle_PB_Content_Block_styles' );
		add_action( 'admin_print_footer_scripts', 'Pootle_PB_Content_Block_footer_scripts' );
	}

}

add_action( 'admin_init', 'pp_page_builder_Pootle_PB_Content_Block_admin_init' );
 ...
 * @param $page
 */
function pp_page_builder_Pootle_PB_Content_Block_admin_enqueue( $page ) {
	...
	if ( ( $screen->base == 'post' && in_array( $screen->id, siteorigin_panels_setting( 'post-types' ) ) ) || $screen->base == 'appearance_page_so_panels_home_page' ) {
		...
		global $Pootle_PB_Content_Block_version;
		if ( ! isset( $Pootle_PB_Content_Block_version ) ) {
//			if ( function_exists( 'black_studio_tinymce_get_version' ) ) {
//				$Pootle_PB_Content_Block_version = black_studio_tinymce_get_version();
			//}
			$Pootle_PB_Content_Block_version = '1.0.0';
		}

//		if ( version_compare( $Pootle_PB_Content_Block_version, '1.3.3', '<=' ) ) {
...
//		if ( version_compare( $Pootle_PB_Content_Block_version, '1.2.0', '<=' ) ) {
...
add_action( 'admin_enqueue_scripts', 'pp_page_builder_Pootle_PB_Content_Block_admin_enqueue', 15 );
