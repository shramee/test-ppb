(function ($) {
        ...
        if (typeof $$.data('dialog') != 'undefined' && !$$.data('dialog').hasClass('ui-dialog-content-loading')) {
            $$.data('dialog').find('*[name]').not('[data-info-field]').each(function () {
                ...
                parts = parts.map(function (e) {
                    if (!isNaN(parseFloat(e)) && isFinite(e))return parseInt(e); else return e;
                });
                var sub = data;
    ...
    $.fn.panelsCreatePanel = function (type, data) {
        ...
        var activeDialog;
        panel.attr('data-type', type).append($('<input type="hidden" name="widgets[' + newPanelId + '][data]" type="hidden">').val(JSON.stringify(data))).append($('<input type="hidden" name="widgets[' + newPanelId + '][info][raw]" type="hidden">').val(0)).append($('<input type="hidden" name="widgets[' + newPanelId + '][info][grid]" type="hidden">')).append($('<input type="hidden" name="widgets[' + newPanelId + '][info][cell]" type="hidden">')).append($('<input type="hidden" name="widgets[' + newPanelId + '][info][id]" type="hidden">').val(newPanelId)).append($('<input type="hidden" name="widgets[' + newPanelId + '][info][class]" type="hidden">').val(type)).append($('<input type="hidden" name="panel_order[]" type="hidden">').val(newPanelId)).data({
            ...
            return false;
        }).end().find('.description').html($$.find('.description').html()).end().find('.title h4').html($$.find('h3').html());
        panel.panelsSetPanelTitle(data);
        var deleteFunction = function () {
            panels.undoManager.register(this, function (type, data, container, position) {
            ...
            }, [panel.attr('data-type'), panel.panelsGetPanelData(), panel.closest('.panels-container'), panel.index()], 'Remove Panel');
            $('#panels-undo-message').remove();
            $('<div id="panels-undo-message" class="updated"><p>' + panels.i10n.messages.deleteWidget + ' - <a href="#" class="undo">' + panels.i10n.buttons.undo + '</a></p></div>').appendTo('body').hide().slideDown().find('a.undo').click(function () {
            ...
            if (panels.animations)panel.slideUp(remove); else {
                panel.hide();
                remove();
            }
        };
        panel.find('.title .actions').append($('<a>' + panels.i10n.buttons.edit + '<a>').addClass('edit').click(function () {
            ...
            activeDialog = $('<div class="panel-dialog dialog-form"></div>').data('widget-type', type).addClass('ui-dialog-content-loading').addClass('widget-dialog-' + type.toLowerCase()).dialog({
            ...
            return false;
        })).append($('<a>' + panels.i10n.buttons.duplicate + '<a>').addClass('duplicate').click(function () {
            ...
            return false;
        })).append($('<a>' + panels.i10n.buttons.delete + '<a>').addClass('delete').click(function () {
            deleteFunction();
            return false;
        }));
        return panel;
    ...
    panels.addPanel = function (panel, container, position, animate) {
        ...
        if (position == null)container.append(panel); else {
            var current = container.find('.panel').eq(position);
            if (current.length == 0)container.append(panel); else {
        ...
        if (animate) {
            if (panels.animations)
                $('#panels-container .panel.new-panel').hide().slideDown(450, function () {
                    panel.find('a.edit').click()
                }).removeClass('new-panel'); else {
    ...
    $.fn.panelsSetPanelTitle = function (data) {
        return $(this).each(function () {
            ...
            if (typeof data != 'undefined') {
                if (typeof data.title != 'undefined' && data.title != '')titleValue = data.title; else {
                    for (var i in data) {
            ...
            for (var gi in data.grids) {
                ...
                for (var pi in data.widgets) {
                    ...
                    if (Number(data.widgets[pi]['info']['grid']) == gi) {
                        ...
                        grid.find('.panels-container').eq(Number(data.widgets[pi]['info']['cell'])).append(panel)
                    }
                }
            }
        }
        $('#panels-container .panels-container').sortable('refresh').trigger('refreshcells');
        $('#panels-container .panel').removeClass('new-panel');
