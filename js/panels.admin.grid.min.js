(function ($) {
        ...
        return $(this).each(function () {
            ...
            if ($$.find('.grid .cell').length > 1) {
                $$.find('.grid .cell').each(function () {
                    if ($(this).is('.first, .last'))totalWidth -= 6; else totalWidth -= 12;
                });
            }
            var left = 0;
            var maxHeight = 0;
            $$.find('.grid .cell').each(function () {
                ...
                $(this).width(Math.floor(totalWidth * Number($(this).attr('data-percent')))).css('left', left);
                left += $(this).width() + 12;
            });
    ...
    panels.createGrid = function (cells, weights, style) {
        ...
        container.append($('<div class="controls" />').append($('<div class="ui-button ui-button-icon-only"><div class="ui-icon ui-icon-remove"></div></div>').attr('data-tooltip', panels.i10n.buttons['delete']).click(function () {
            $(this).removeTooltip();
            var containerData = [];
            container.find('.cell').each(function (i, el) {
                containerData[i] = {'weight': Number($(this).attr('data-percent')), 'widgets': []};
            ...
            window.panels.undoManager.register(this, function (containerData, position) {
                var weights = [];
                ...
                if (panels.animations)gridContainer.hide().slideDown(); else gridContainer.show();
            }, [containerData, container.index()], 'Remove Columns');
            $('#panels-undo-message').remove();
            $('<div id="panels-undo-message" class="updated"><p>' + panels.i10n.messages.deleteColumns + ' - <a href="#" class="undo">' + panels.i10n.buttons.undo + '</a></p></div>').appendTo('body').hide().fadeIn().find('a.undo').click(function () {
            ...
            var remove = function () {
                container.remove();
                $('#panels-container').sortable("refresh").find('.panels-container').trigger('refreshcells');
            };
            if (panels.animations)container.slideUp(remove); else {
                container.hide();
                remove();
            }
            return false;
        })).append($('<div class="ui-button ui-button-icon-only panels-visual-style"><div class="ui-icon ui-icon-gear"></div></div>').attr('data-tooltip', 'Visual Style').click(function () {
        ...
        for (var i = 0; i < cells; i++) {
            var cell = $('<div class="cell" data-percent="' + (weights[i] / weightSum) + '">' + '<div class="cell-wrapper panels-container"></div>' + '<div class="cell-width"><div class="cell-width-left"></div><div class="cell-width-right"></div><div class="cell-width-line"></div><div class="cell-width-value"><span></span></div></div>' + '</div>');
            if (i == 0)cell.addClass('first');
            if (i == cells - 1)cell.addClass('last');
            grid.append(cell);
            cell.append($('<input type="hidden" name="grid_cells[' + cellId + '][weight]" />').val(weights[i] / weightSum)).append($('<input type="hidden" name="grid_cells[' + cellId + '][grid]" />').val(gridId)).data('cellId', cellId)
            cellId++;
        }
    ...
    panels.setupGrid = function ($$) {
        ...
        $$.find('.grid .cell').not('.first').each(function () {
            ...
            $(this).resizable({
                handles: 'w', containment: 'parent', start: function (event, ui) {
