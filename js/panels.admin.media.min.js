jQuery(function ($) {
    ...
    wp.media.editor.insert = function (h) {
        ...
        if (h.indexOf('[gallery') !== -1) {
            ...
            var panel = $('#panels-dialog').panelsCreatePanel('SiteOrigin_Panels_Widgets_Gallery', {'ids': ids.join(',')});
            if (panel == null)originalInsert(h); else panels.addPanel(panel, null, null, true);
            return;
        }
            ...
            if ($el.prop("tagName") == 'A' && $el.children().eq(0).prop('tagName') == 'IMG') {
                ...
                panel = $('#panels-dialog').panelsCreatePanel('SiteOrigin_Panels_Widgets_Image', {'src': $el.attr('src')});
            }
            else if ($el.prop('tagName') == 'A' && ($el.attr('href').indexOf('.mp4') !== -1 || $el.attr('href').indexOf('.avi') !== -1)) {
                panel = $('#panels-dialog').panelsCreatePanel('SiteOrigin_Panels_Widgets_Video', {'url': $el.attr('href')});
            }
            if (panel == null)originalInsert(h); else panels.addPanel(panel, null, null, true);
            return;
        }
