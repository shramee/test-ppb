jQuery(function ($) {
    ...
    $('#panels-dialog').show().dialog({
        ...
        close: function () {
            $(this).data('overlay').remove();
            if (panels.animations)$('#panels-container .panel.new-panel').hide().slideDown(1000).removeClass('new-panel'); else $('#panels-container .panel.new-panel').show().removeClass('new-panel');
        }
    }).on('keydown', function (e) {
    ...
    $('#panels .panels-add').button({icons: {primary: 'ui-icon-add'}, text: false}).click(function () {
        $('#panels-text-filter-input').val('').keyup();
        $('#panels-dialog').dialog('open');
        return false;
    });
    $('#panels .grid-add').button({icons: {primary: 'ui-icon-columns'}, text: false}).click(function () {
        $('#grid-add-dialog').dialog('open');
        return false;
    });
    $('#siteorigin-widgets-link').data('text', $('#siteorigin-widgets-link').html());
    $('#panels-text-filter-input').keyup(function (e) {
        if (e.keyCode == 13) {
        ...
        $('#panels-dialog .panel-type-list .panel-type').show().each(function () {
            if (value == '')return;
    ...
    if (typeof panelsData != 'undefined')panels.loadPanels(panelsData); else panels.createGrid(1);
    ...
    }).end().prepend($('<a id="content-panels" class="hide-if-no-js wp-switch-editor switch-panels">' + $('#so-panels-panels h3.hndle span').html() + '</a>').click(function () {
        var $$ = $(this);
    ...
    $('#so-panels-panels').insertAfter('#wp-content-editor-container').addClass('wp-editor-container').hide().find('.handlediv').remove().end().find('.hndle').html('').append($('#add-to-panels'));
    ...
    if ($('#panels-home-page').length) {
        ...
        $('#content-panels').hide();
        $('#panels-toggle-switch').mouseenter(function () {
